...
Onboard Go GAPIC client library generation to the Librarian 0.2 platform, replacing the legacy [bazel-bot](https://github.com/googleapis/repo-automation-bots/tree/main/packages/bazel-bot), [OwlBot](http://go/owlbot-howto#githubowlbotyaml) and [googleapis-gen](https://github.com/googleapis/googleapis-gen) toolchain with a modern, container-based solution.
...
1. **Generation:** `bazel-bot` [invokes](https://github.com/googleapis/repo-automation-bots/blob/main/packages/bazel-bot/docker-image/generate-googleapis-gen.sh) [protoc](https://protobuf.dev/getting-started/gotutorial/) via Bazel to generate Go source code from the public API definitions in [googleapis](https://github.com/googleapis/googleapis) into a private intermediate repository, `googleapis-gen`.
2. **File placement:** `OwlBot` opens a PR to copy the generated code from `googleapis-gen` into the final `google-cloud-go` repository, applying rules for file inclusion and preservation defined in a central [.OwlBot.yaml](https://github.com/googleapis/google-cloud-go/blob/main/.github/.OwlBot.yaml) file.
3. **Post-processing:** In the PR, a Go-specific [OwlBot post-processor](https://github.com/googleapis/google-cloud-go/tree/v0.121.3/internal/postprocessor) runs to perform cleanup, apply code modifications, and generate additional files (like `go.mod`, `README.md` and `version.go`) that are not handled by the core `protoc` generators.
...
If possible, the container should be hosted in an [Airlock 1P repository](https://g3doc.corp.google.com/company/teams/artifact-registry-airlock/howto_first_party_repository.md).
...
Librarian copies files from the `/output` directory into the repository based on the `source_paths` configured for the library in `state.yaml`. This copy operation is destructive: if a `source_paths` entry is set to a directory (e.g., `workflows`), any files or subdirectories within `<repo>/workflows` that are *not* present in `/output/workflows` will be deleted.
...
Let's look at the details for our `workflows` example. The paths in `generate-request.json` may include nested APIs as in this configuration for `workflows`, which is based on the proto definitions in [googleapis/google/cloud/workflows](https://github.com/googleapis/googleapis/tree/master/google/cloud/workflows):
...
Module-scope (not global) files modified by the current post-processor:
...
3. **Docker-based E2E testing:** An automated script (`run-container-tests.sh`) will execute the full, containerized workflow for a suite of test libraries, covering important edge cases, including error cases. A test is successful if:
...
1. Onboard a single Librarian library.
   ...
   * If ready, configure Librarian releases for thefor the the library.
   * Verify that Librarian correctly releases the library.
   * Remove the library from the OwlBot and release-please configurations.
2. Repeat the steps above for the eight "difficult" libraries below. This group is defined in gapic-generator-go's [test.sh](https://github.com/googleapis/gapic-generator-go/blob/main/test.sh).
   * `datastore`
   * `kms`
   * `datacatalog`
   * `texttospeech`
   ...
   * `apigeeconnect`
   * `bigquery`
3. Repeat the steps above for all remaining libraries in google-cloud-go.  
4. Remove all OwlBot and release-please configuration.
