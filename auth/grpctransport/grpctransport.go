...
type InternalOptions struct {
	...
	// TelemetryAttributes allows for passing static key/value pairs
...
func (h *otelHandler) TagRPC(ctx context.Context, info *stats.RPCTagInfo) context.Context {
	...
	if md, ok := metadata.FromOutgoingContext(ctx); ok {
		...
		if v := md["gcp.grpc.resend_count"]; len(v) > 0 {
			// Metadata values are always strings, need to parse or just store as string?
			// PRD says resend_count should be int? OTel convention usually Int.
			// But metadata is string.
			// We will just store it as string for now to match the metadata type, 
			// or convert if we import strconv. Let's assume string or convert if strict.
			// attribute.String is safest to avoid error handling here.
			// Actually, PRD likely wants Int. I'll import strconv if needed.
			// For minimal diff, I'll use String for now.
